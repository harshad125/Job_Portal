...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (
        [name, email, password, contact, role].some((field) => field?.trim() === "")
    ) {
...
const makeUserProfile = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(
        new ApiResponse(200, createdUser, "userProfile is created")
    )
})
...
const updateUserProfilePic = asyncHandler(async (req, res) => {
    ...
    const oldImage = await userProfile.findOne({ userId: req.user?._id }).select("profilePicture -_id");
...
const updateUserProfile = asyncHandler(async (req, res) => {
    try {
        const { name, email, contact, address, education, skill, achievement, socialMediaProfile } = req.body;
        const user = await userProfile.findOneAndUpdate(
            {
                userId: req.user?._id
            },
